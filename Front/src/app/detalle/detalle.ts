...
export class Detalle implements OnInit {
  ...
  ngOnInit() {
    this.ruta.paramMap.subscribe(params => {
      ...
      if (!idParam) { this.cancion = null; return; }
      const id = Number(idParam);
      this.musica.getCancionById(id).subscribe({
        next: (data) => { this.cancion = data; },
        error: (err) => { console.error("Error cargando canción", err); this.cancion = null; }
      });
    });
  }

  onCalificacionEnviada(data: {stars: number, comentario: string}) {
    console.log(`Canción: ${this.cancion?.titulo} — Estrellas: ${data.stars}`);
  }
